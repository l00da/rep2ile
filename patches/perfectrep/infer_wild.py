"""
Drop-in replacement for AndrewBoessen/PerfectRep `infer_wild.py` (repo root).

RepTile patch (video-free inference):
  - `--video` is optional. When omitted, use `--fps` (default 30) for metadata only;
    skip `render_and_save` (no `X3D.mp4`). Always write `X3D.npy`.
  - When `--video` is provided, behavior matches upstream (renders mp4, uses video FPS/size).

Copy over your clone after backing up the original:
...
def parse_args():
    ...
    parser.add_argument('-v', '--video', type=str, default=None, help='video path (optional for JSON-only inference)')
    parser.add_argument('--fps', type=float, default=30.0, help='FPS when --video is omitted (JSON-only mode)')
...
if args.pixel and not args.video:
    raise SystemExit(
        'infer_wild.py: --pixel requires --video for frame dimensions. '
        'JSON-only mode supports normalized coordinates only (--pixel off).'
    )
...
if args.pixel:
    ...
    wild_dataset = WildDetDataset(args.json_path, clip_len=243, vid_size=vid_size, scale_range=[1,1], focus=args.focus)
